...
void loop()
{
  ...
  for (int i= 0; i< 60; i++){ // nombre arbitraire d'affichage différent de l'equalizer (30 sec d'animation prévu)
    for (int w =0; w< width ; w++){
      hcolonne =  random (0, height);  // hauteur aléatoire de la colonne
      
    rgb[0] = random (0, 255);
    rgb[1] = random (0, 255);
    rgb[2] = random (0, 255); // couleur de la colonne définie aléatoirement
   ...
   BlinkM_setRGB( matrix[w][h], rgb[0], rgb[1], rgb[2]); 
     ...
     for ( int wid= 0; wid< 60; w++){ // On éteint toutes les leds
